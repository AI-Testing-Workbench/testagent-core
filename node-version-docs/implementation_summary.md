...
### 3. `packages/opencode-server/script/build.ts`
- 自动化构建脚本
- 步骤：
...
### 4. `packages/opencode-server/README.md`
- 完整的使用文档
- 包含安装、启动、配置说明
- SDK 连接示例
- 功能列表

### 5. `packages/opencode-server/.gitignore`
...
- `opencode-server` 的构建脚本直接复制整个 `dist/node/` 目录
...
```bash
# 将 dist/ 目录复制到目标服务器
scp -r packages/opencode-server/dist/ user@server:/opt/opencode-server/
