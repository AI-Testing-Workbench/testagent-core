...
function isAutoMemoryPart(part) {
  ...
  return typeof part.text === "string" && part.text.includes("# Auto Memory")
}
...
function startRecallPrefetch(llm, sessionID, turnID, worktree, query, alreadySurfaced, recentTools, model) {
  if (!llm || !isUsefulRecallQuery(query)) return undefined
  const handle = {
    turnID: turnID,
    settled: false,
    consumed: false,
  ...
  const promise = findRelevantMemories(
    createOpenCodeRecallLLMClient(llm, sessionID || ""),
    sessionID || "",
    worktree,
    query,
    config()?.recall?.llmRecall ?? false,
    alreadySurfaced,
    recentTools,
    model,
  )
...
export const MemoryPlugin = async (params) => {
  ...
  async function appendBufferMessage(part) {
    try {
      ...
      const key = `${part.sessionID}:${messageID}:${text}`
  ...
  return {
    ...
    event: async ({ event }) => {
      if (event.type === "message.updated") {
        ...
        if (config().enable) {
          ...
          try {
            ...
            log.warn(
              `message.updated: failed to fetch message ${msg.id} for session ${msg.sessionID.substring(0, 16)}:`,
              e,
            )
          }
        }
      ...
      if (event.type === "message.part.updated") {
        ...
        if (config().enable) {
          try {
            // 子Agent的子session 跳过
            if (await shouldSkip(part.sessionID)) return
            await appendBufferMessage(part)
          } catch (e) {
            // Message may not be fetchable yet during streaming
            log.warn(
              `message.part.updated: failed to fetch part ${part.id} from message ${part.messageID} for session ${part.sessionID.substring(0, 16)}:`,
              e,
            )
          }
        }
      ...
      if (event.type === "session.idle") {
        ...
        if (config().enable) {
          ...
          log.info(
            `[session.idle] trigger now, session id: ${sessionID} , turnsSinceCuration is: ${state.turnsSinceCuration}`,
          )
    ...
    "experimental.session.compacting": async (_input, output) => {
      // 提取 sessionID
      const sessionID = _input && typeof _input === "object" && "sessionID" in _input ? _input.sessionID : undefined
    ...
    "experimental.chat.messages.transform": async (_input, output) => {
      ...
      if (sessionID && isWorkerSession(sessionID)) return
      ...
      if (sessionID) {
        ...
        const newCtx = {
          ...
          alreadySurfaced,
          recentTools,
          prevMessageCount: output.messages.length,
          isLoadSystemPrompt,
          turnID,
          recallPrefetch,
        }
    ...
    "experimental.chat.system.transform": async (_input, output) => {
      ...
      if (_input && typeof _input === "object") {
        sessionID = typeof _input.sessionID === "string" ? _input.sessionID : undefined
      ...
      //   alreadySurfaced.add(key)
      // }
      const memoryPrompt = buildMemorySystemPrompt(worktree, recalledSection, isLoadSystemPrompt, {
    ...
    tool: {
      ...
      memory_personal_save: tool({
        description:
          "Save or update personal global memory file." +
