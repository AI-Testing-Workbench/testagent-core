...
//   return memoryDir
// }
// 当前目录/.testagent/memory
export function getMemoryDir(worktree) {
    const memoryDir = join(worktree, ".testagent", "memory");
    ensureDir(memoryDir);
    return memoryDir;
}
// 团队记忆目录
export function getTeamMemoryDir(worktree) {
    ...
    ensureDir(teamMemoryDir);
    return teamMemoryDir;
}
export function getTeamMemoryEntrypoint(worktree) {
    return join(getTeamMemoryDir(worktree), ENTRYPOINT_NAME);
}
...
// testagent 配置目录
// ~/.config/opencode
export function getOpencodeConfigHomeDir() {
    return join(homedir(), ".config", "testagent").normalize("NFC");
}
// 自定义命令目录
// ~/.config/opencode/commands
export function getOpencodeConfigCommands() {
    const cmdDir = join(getOpencodeConfigHomeDir(), "commands").normalize("NFC");
    ensureDir(cmdDir);
    return cmdDir;
}
/**
